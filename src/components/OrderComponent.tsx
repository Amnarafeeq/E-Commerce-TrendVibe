                    ...
                    <Tooltip key={order.orderNumber}>
                        <TooltipTrigger asChild>
                           <TableRow className='cursor-pointer hover:bg-gray-100 h-12 ' onClick={()=>setSelectedOrder(order)}>
                            ...
                            <TableCell className='font-medium '><PriceFormatter amount={order.totalPrice}/></TableCell>
                            <TableCell>{order.status && <span className={`px-2 py-1 rounded-full text-xs font-semibold ${order.status === "paid" ? "bg-green-100 text-green-800 ":"bg-yellow-100 text-yellow-800" }`}>{order.status}</span> }</TableCell>
                            <TableCell className='hidden md:table-cell'>{order.invoice && <p>{order.invoice? order.invoice.number : "---"}</p> }</TableCell>
                            </TableRow> 
                        </TooltipTrigger>
