...
const SideBar = ({ isOpen, onClose }: Props) => {
  ...
  return (
    <div
      className={`fixed inset-y-0 left-0 z-50 bg-black/50 shadow-xl hoverEffect w-full ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
    >
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 0.4, delay: 0.3 }}
        className="min-w-72 max-w-96 bg-white h-full p-10 border-r border-r-black flex flex-col gap-6"
      >
          ...
          {headerData.map((item, index) => {
            return (
              <Link
                ...
                href={item.href}
                key={index}
                className={`text-textColor1 hover:text-textColor2 hoverEffect ${pathname === item.href && "text-darkBackground"}  `}
              >
                {item.title}
                <span
