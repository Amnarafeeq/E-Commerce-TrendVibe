    ...
    <header className=" sticky top-0 z-50 bg-lightBackground shadow-lg border-b border-b-gray-300 py-5">
      <Container className="flex justify-between items-center  gap-7">
        <div className="flex items-center ml-3 gap-2">
          <MobileMenu />
          <h2 className="text-xl sm:text-[24px] font-bold text-darkBackground">
            TrendVibe
          </h2>
        </div>
          ...
          <ClerkLoaded>
            <SignedIn>
              <Link href="/orders" className="group relative">
                ...
                <span className="absolute -top-1 -right-1 bg-darkBackground text-textColor2 h-4 w-4 rounded-full text-xs font-semibold flex items-center justify-center">
            ...
            {!users && (
              <SignInButton mode="modal">
                <button className="text-sm font-semibold text-textColor2 ">
                  Login
                </button>
              </SignInButton>
            )}
